        ...
        <ViewOnlySection title="Overview">
          <div className="space-y-4">
            <div className="flex min-w-0 flex-wrap items-center gap-2">
              ...
              <span className={`rounded-full px-2.5 py-1 text-xs font-medium ${priorityStyle(opportunity.priority)}`}>
                {opportunity.priority}
              </span>
              <span className={`rounded-full px-2.5 py-1 text-xs font-medium ${stageStyle(opportunity.stage)}`}>
                {opportunity.stage}
              </span>
            </div>
...
export function OpportunityReadOnlyModal({
  ...
  const footer = (
    <div className="flex gap-3">
      ...
      {onEdit && (
        <button type="button" onClick={onEdit} className={`flex-1 ${modalButtonSecondary}`}>
          Edit
        </button>
      )}
      <button type="button" onClick={onClose} className={`${modalButtonPrimary} ${onEdit ? 'flex-1' : 'w-full'}`}>
        Close
      </button>
    </div>
  );
